        ...
        for path in tqdm.tqdm(paths):
            ...
            if need_reformat == 1:
                ...
                tmp_path = "%s/%s.reformatted.wav" % (
                    os.path.join("C:/Users/Administrator/UntitledProjects/GPT-SoVITS/output/uvr5_opt"),
                    os.path.basename(inp_path),
                )
                os.system(
                    f'ffmpeg -i "{inp_path}" -vn -acodec pcm_s16le -ac 2 -ar 44100 "{tmp_path}" -y -loglevel quiet'
                )
                inp_path = tmp_path
            try:
                if done == 0:
                ...
                with gr.Row():
                    ...
                    with gr.Column():
                        ...
                        opt_vocal_root = gr.Textbox(
                            label="指定输出主人声文件夹", value="output/uvr5_opt"
                        )
                        opt_ins_root = gr.Textbox(
                            label="指定输出非主人声文件夹", value="output/uvr5_opt"
                        )
                        format0 = gr.Radio(
                            label="导出文件格式",
