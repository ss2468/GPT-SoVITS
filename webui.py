...
with gr.Blocks(title="GPT-SoVITS WebUI") as app:
    ...
    with gr.Tabs():
        with gr.TabItem(i18n("0-前置数据集获取工具")):  # 提前随机切片防止uvr5爆内存->uvr5->slicer->asr->打标
            ...
            with gr.Row():
                with gr.Row():
                    slice_inp_path = gr.Textbox(label=i18n("音频自动切分输入路径，可文件可文件夹"), value=switch_path("output/uvr5_opt"))
                    slice_opt_root = gr.Textbox(label=i18n("切分后的子音频的输出根目录"), value=switch_path("output/slicer_opt"))
            ...
            with gr.Row():
                ...
                denoise_output_dir = gr.Textbox(label=i18n("降噪结果输出文件夹"), value=switch_path("output/denoise_opt"))
                ...
                with gr.Column():
                    with gr.Row():
                        asr_inp_dir = gr.Textbox(
                            label=i18n("输入文件夹路径"),
                            value=switch_path("output/slicer_opt"),
                            interactive=True,
                        )
                        asr_opt_dir = gr.Textbox(
                            label=i18n("输出文件夹路径"),
                            value=switch_path("output/asr_opt"),
                            interactive=True,
                        )
                    with gr.Row():
            ...
            with gr.Row():
                ...
                path_list = gr.Textbox(
                    ...
                    value=switch_path("output/slicer_opt/slicer_opt.list"),
                    interactive=True,
                )
        ...
        with gr.TabItem(i18n("1-GPT-SoVITS-TTS")):
            ...
            with gr.TabItem(i18n("1A-训练集格式化工具")):
                ...
                with gr.Row():
                    inp_text = gr.Textbox(label=i18n("*文本标注文件"), value=switch_path("output/slicer_opt/slicer_opt.list"), interactive=True)
